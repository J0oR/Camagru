(function()
{
    ...
    function startup()
    {
        ...
        navigator.mediaDevices.getUserMedia({video: true, audio: false }).then(handleSuccess).catch(handleError);
        ...
        var takep_func = function(ev)
        {
            if (validate())
            {
                ...
                document.getElementById("unselected_super").style.display = 'block';
...
function previewFile()
{
    ...
    var upload_wrapper = document.getElementById('upload_wrapper');
...
function check_size() {
    ...
    if (a.length)
    {
        ...
        if ((a_type !== "image/jpg" && a_type !== "image/png" && a_type !== "image/jpeg" && a_type !== "image/gif") || (a[0].size > max_size)) {
            document.getElementById('noCamera').style.display='none!important';
