                ...
                if ($currentpage > 1)// if not on page 1, don't show back links
                {
                    ...
                    <li>
                        <form method="POST" action="<?= URL_WITH_INDEX_FILE . 'gallery' ?>">
                            ...
                            <button class="pagination-link" type="submit">First Page</button>
                    ...
                    <li>
                        <form method="POST" action="<?= URL_WITH_INDEX_FILE . 'gallery' ?>">
                            ...
                            <button type="submit" class="pagination-link">Previous</button>
                        </form>
                    </li>
                    <?php
                }
                if ($totalpages > 1)
                {
                    ...
                    for ($x = ($currentpage - $range); $x < (($currentpage + $range) + 1); $x++)
                    {
                        ...
                        if ($x > 0 AND $x <= $totalpages AND $x == $currentpage): ?>
                            <li>
                                <button class="pagination-link is-current"><?= htmlspecialchars($x) ?></button>
                            </li>
                ...
                if ($currentpage != $totalpages)
                {
                    ...
                    <li>
                        <form method="POST" action="<?= URL_WITH_INDEX_FILE . 'gallery' ?>">
                            ...
                            <button class="pagination-link" type="submit">Next</button>
                        </form>
                    </li>
                    <li>
                        <form method="POST" action="<?= URL_WITH_INDEX_FILE . 'gallery' ?>">
                            <input type="hidden" name="currentpage" value="<?= htmlspecialchars($totalpages, ENT_QUOTES) ?>">
                            <button class="pagination-link" type="submit">Last Page</button>
                        </form>
                    </li>
                    <?php
                }
